...
namespace Fungus {
    ...
    public class ControlAudio : Command {
        ...
        [SerializeField] protected FloatData startVolume = new FloatData (1f);

        //[Range(0f,1f)]
        [Tooltip ("End audio at this volume")]
        [SerializeField] protected FloatData endVolume = new FloatData (1f);
        ...
        protected virtual void PlayLoop () {
            if (fadeDuration > 0) {
                ...
                LeanTween.value (_audioSource.Value.gameObject, 0, endVolume, fadeDuration).setOnUpdate (
                    (float updateVolume) => {
                        _audioSource.Value.volume = updateVolume;
                    }
                ).setOnComplete (
        ...
        protected virtual void PauseLoop () {
            if (fadeDuration > 0) {
                LeanTween.value (_audioSource.Value.gameObject, _audioSource.Value.volume, 0, fadeDuration).setOnUpdate (
                    (float updateVolume) => {
                        _audioSource.Value.volume = updateVolume;
                    }
                ).setOnComplete (
        ...
        protected virtual void StopLoop (AudioSource source) {
            if (fadeDuration > 0) {
                LeanTween.value (source.gameObject, _audioSource.Value.volume, 0, fadeDuration).setOnUpdate (
                    (float updateVolume) => {
        ...
        protected virtual void ChangeVolume () {
            LeanTween.value (_audioSource.Value.gameObject, _audioSource.Value.volume, endVolume, fadeDuration).setOnUpdate (
                (float updateVolume) => {
                    _audioSource.Value.volume = updateVolume;
                }).setOnComplete (
                () => {
